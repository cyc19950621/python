...
import re
import pandas as pd
f = open(r'D:\DeepLearning ER\EHR_all.txt','r',errors="ignore")#年龄-疾病关联
f = open(r'D:\DeepLearning ER\EHRzhzd6\0.txt','r',errors="ignore")#中文加入errors
ryzd = []
a= f.readlines()
for line in a:
    line=re.sub('\n','',line)
    ryzd.append(line.split())
...
def ResultDFToSave(rules):   #根据Qrange3关联分析生成的规则得到并返回对于的DataFrame数据结构的函数
    ...
    return pd.DataFrame(returnRules,columns=('规则','项集出现数目','置信度','支持度','力度','提升度','利用度'))
...
# 下面将结果保存成excel格式的文件    
    ...
    dfToSave.to_excel(r'C:\Users\Administrator\Desktop\3.xlsx')
